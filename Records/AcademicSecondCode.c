...
#include <string.h>
#define GRADES 10
char  academic_registry[GRADES][9];
int   grade_1[GRADES];
int   grade_2[GRADES];
...
int duplicate_ra(int quantity, const char *r) {
    for (int i = 0; i < quantity; i++)
        if (strcmp(academic_registry[i], r) == 0) return 1;
    return 0;
}
...
void register_student(int *quantity) {
    if (*quantity >= GRADES) { printf("Class full!\n"); return; }
    int i = *quantity;
    do {
        printf("RA (numbers only, up to 8 digits): ");
        scanf("%8s", academic_registry[i]);
        if (!valid_ra(academic_registry[i]))
            printf("  Invalid RA! Use only numeric digits.\n");
        else if (duplicate_ra(i, academic_registry[i]))
...
void list(int qty) {
    ...
    for (int i = 0; i < qty; i++) {
        ...
        printf("%-10s %4d %4d %s %7.1f  %s\n",
            academic_registry[i], grade_1[i], grade_2[i], eg, average[i], status[i]);
    }
}
int main(void) {
    int quantity = 0, op;
    do {
        ...
        scanf("%d", &op);
        if (op == 1) register_student(&quantity);
        if (op == 2) list(quantity);
    } while (op != 0);
    return 0;
}
